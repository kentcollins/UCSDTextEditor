...
public class DocumentBenchmarking {
	...
	public static void main(String[] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps * increment + start; numToCheck += increment) {
			...
			String onlyNumChars = getStringFromFile(textfile, numToCheck);
			long basicTimeStart = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				Document basic = new BasicDocument(onlyNumChars);
				basic.getFleschScore();
			}
			long basicTimeStop = System.nanoTime();
			System.out.print((basicTimeStop - basicTimeStart) + "\t");
			long efficientTimeStart = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				Document efficient = new EfficientDocument(onlyNumChars);
				efficient.getFleschScore();

			}
			long efficientTimeStop = System.nanoTime();
			System.out.print((efficientTimeStop - efficientTimeStart) + "\n");
		}
